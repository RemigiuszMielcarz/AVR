...
const uint8_t cyfry[15] = {
		~(SEG_A|SEG_B|SEG_C|SEG_D|SEG_E|SEG_F),			    //0		~0011 1111 	czyli 1100 0000	podłączone anodami do masy
...
void init(void)
{
	...
	OCR0 = 53; 				// Dodatkowy podzial przez 53	(11059200 Hz / 1024 / 200Hz[oczekiwana wartośc])-1 = 53
...
ISR(TIMER2_OVF_vect)
{
	...
	if(licznik_ovf == 125)
    {
		if(odliczanie == 1 && wartosc != 0) // Jeśli jest zezwolenie na odliczanie i zmierzona wartość nie jest jeszcze zerem to:
            {
			    wartosc--; 		    //Zacznij odejmować do zera
		    }
		licznik_ovf = 0;
	}
}
...
int main(void)
{
    ...
    // Sprawdzenie czy działa wyświetlanie liczb
	cyfra[0] = 0;
	cyfra[1] = 8;
	cyfra[2] = 8;
	_delay_ms(2500);
    ...
    //Sprawdzenie działania diody LED
    ON_LED;
    _delay_ms(2000);
    OFF_LED;
